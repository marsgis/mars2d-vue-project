...
Mars2D 基础项目 是基于[Mars2D 平台](http://mars2d.cn)做的一个应用系统，提供的一个基础项目模版，包含常用基础地图功能，可在该基础项目上快速开发搭建新项目。方便快速搭建三维地图产品，敏捷开发，可复用，支持各种配置，适合各种场景使用。
...
```
git clone git@github.com:marsgis/mars2d-vue-project.git
```

- [Gitee](https://gitee.com/marsgis/mars2d-vue-project)：国内码云，下载速度快些。

```
git clone git@gitee.com:marsgis/mars2d-vue-project.git
```
...
![image](http://mars2d.cn/dev/img/guide/project-vue-liu.jpg)
...
3. map.ts 主动触发 ui 变化，通过 mars2d.BaseClass 事件中心处理。如
...
可以参考已集成好的mars2d-vue-project-admin项目:
[github:vue-mars2d-admin](https://github.com/marsgis/mars2d-vue-project-admin)、
[gitee:vue-mars2d-admin](https://gitee.com/marsgis/mars2d-vue-project-admin)
...
![image](http://mars2d.cn/dev/img/guide/project-vue-hebing.jpg)
...
基础项目已经基本保证不会影响外部样式，此处要处理的是您项目中的全局样式对 mars2d 相关组件的影响。修改相关 CSS 保证基础项目功能 UI 正常即可。
...
- 将离线包内文件 覆盖至`src\common\mars2d-sdk\dist\`目录下。
- 修改webpack或vue.config等相关配置文件中，增加下面配置
```js 
//已忽略其他配置
chainWebpack: (config) => {
    config.resolve.alias.set("mars2d", resolve("src/common/mars2d-sdk"))
},
```
- 修改`package.json`删除mars2d包，增加@turf/turf mars2d-cesium包
```json
//已忽略其他配置
"dependencies": {
  "@turf/turf": "^6.5.0",
